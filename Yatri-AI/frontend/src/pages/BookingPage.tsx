        ...
        {step === 0 && (
          <motion.div key="step0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <div className="glass-card" style={{ marginBottom: 24 }}>
              ...
              {selectedRoute.segments.map((seg, i) => (
                <div key={i} style={{ marginBottom: 12, padding: '10px', background: 'var(--bg-base)', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {seg.bookingUrl && (
                      <button 
                        onClick={() => window.open(seg.bookingUrl, '_blank')}
              ...
              <div style={{ borderTop: '1px solid var(--border-default)', paddingTop: 12, paddingBottom: 16, display: 'flex', justifyContent: 'space-between', fontWeight: 700 }}>
                <span>Total</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
                <button
                  onClick={() => {
                    ...
                    toast.success(
                      'Opening your booking links! If all tabs do not open, please click "Allow Pop-ups" in your browser\'s address bar.',
                      { duration: 6000 }
                    );
                    
                    const urls = new Set<string>();
                    selectedRoute.segments.forEach(seg => {
                      if (seg.bookingUrl) urls.add(seg.bookingUrl);
                    });
