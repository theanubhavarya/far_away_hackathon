...
if st.button("Find Best Route", use_container_width=True, type="primary"):
    ...
    with st.spinner("Analysing routes using Deep Learning..."):
        try:
            ...
            badges_list = []
            for leg in result['Route Details']:
                t_type = leg.get('Transport_Type', leg.get('mode', 'Cab'))
                t_type_str = str(t_type).capitalize()
                bc = badge_colors.get(t_type_str, 'badge-cab')
                emoji = TRANSPORT_EMOJIS.get(t_type_str, '🚕')
                badges_list.append(f"<span class='badge {bc}'>{emoji} {t_type_str}</span>")
            
            badges_html = " &rarr; ".join(badges_list)

            # Build route path text starting with the origin of the first leg, followed by destinations of all legs
            route_path = []
            if result['Route Details']:
                first_leg = result['Route Details'][0]
                first_source = first_leg.get('Source', first_leg.get('from', ''))
                route_path.append(first_source)
                for leg in result['Route Details']:
                    dest = leg.get('Destination', leg.get('to', ''))
                    route_path.append(dest)
            
            steps_html = "<br>&rarr; ".join(route_path)
